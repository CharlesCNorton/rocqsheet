...
7. **Per-cell pre/postconditions.** A cell carries a Coq `Prop`
...
8. **Unit-typed cells.** Add `Cell::CUnit` carrying `(Unit, Z)`
...
11. **Inline contract checking in the formula bar.** Each column
...
12. **Sheet-level dependent types.** Column `B` declared as
    `Vector R (length A)` indexes `B` by `A`'s row count.
    *Theorems:* well-typedness preserved by every `pure_edit_step`.

13. **Sandboxed scripting language.** Extend `Macros.v` to a typed
...
14. **Property-based regressions on save.** Per-column `Prop`s
...
15. **Static error-reachability analysis.** `analyze_workbook : Sheet
