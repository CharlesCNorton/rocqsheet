...
6. **Inline contract checking in the formula bar.** Each column
...
7. **Sheet-level dependent types.** Column `B` declared as
    `Vector R (length A)` indexes `B` by `A`'s row count.
    *Theorems:* well-typedness preserved by every `pure_edit_step`.

8. **Sandboxed scripting language.** Extend `Macros.v` to a typed
...
9. **Property-based regressions on save.** Per-column `Prop`s
...
10. **Static error-reachability analysis.** `analyze_workbook : Sheet
