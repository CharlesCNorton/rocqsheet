...
6. **Verified financial primitives.** `BLACK_SCHOLES`, `PV`, `IRR`,
...
7. **Inline contract checking in the formula bar.** Each column
...
8. **Sheet-level dependent types.** Column `B` declared as
    `Vector R (length A)` indexes `B` by `A`'s row count.
    *Theorems:* well-typedness preserved by every `pure_edit_step`.

9. **Sandboxed scripting language.** Extend `Macros.v` to a typed
...
10. **Property-based regressions on save.** Per-column `Prop`s
...
11. **Static error-reachability analysis.** `analyze_workbook : Sheet
