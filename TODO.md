...
6. **Lift closed-form smoke theorems to universal forms.** Many
   theorems land as `*_smoke` over specific inputs.  Promote the
   algebraic ones (`eval_add_comm`, `eval_mul_assoc`, `concat_assoc`,
   `negb_involutive` over `EBool`) to universally quantified
   statements where the kernel supports it.

7. **Move `rocq-crane-imgui` into a stand-alone repository.**
...
9. **Hash-chained edit log.** Extend `Concurrent.OpLog` with a
...
10. **Per-cell pre/postconditions.** A cell carries a Coq `Prop`
...
11. **Unit-typed cells.** Add `Cell::CUnit` carrying `(Unit, Z)`
...
17. **Sheet-level dependent types.** Column `B` declared as
    `Vector R (length A)` indexes `B` by `A`'s row count.
    *Theorems:* well-typedness preserved by every `pure_edit_step`.

18. **Sandboxed scripting language.** Extend `Macros.v` to a typed
...
19. **Property-based regressions on save.** Per-column `Prop`s
...
20. **Per-cell provenance hashes.** `value_hash r = H(formula,
    inputs_hashes)`.
    *Theorems:* identical workbooks under identical inputs produce
    identical hashes; collision-resistance reduction to the
    underlying digest.

21. **CRDT convergence proof for the operation log.** Merge over
    `Concurrent.OpLog`.
    *Theorems:* `merge_commutative`, `merge_associative`,
    `merge_idempotent`.

22. **Trade-ticket schema with round-trip.** Record `TradeTicket`
...
23. **Static error-reachability analysis.** `analyze_workbook : Sheet
